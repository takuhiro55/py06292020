...
def setcookie():
    ...
    if request.method == "POST":
        ...
        if request.form.get("foodChoice"): # if nm was assigned via the POST
        #if request.form["nm"] <-- this also works, but returns ERROR if no nm
            food = request.form.get("foodChoice") # grab the value of nm from the POST
        else: # if a user sent a post without nm then assign value defaultuser
            food = "default_banana"
        ...
        # Since you normally just return strings
        # Flask will convert them into response objects for you
        resp2 = make_response(render_template("readcookie1.html"))
        # add a cookie to our response object
                        #cookievar #value
        resp2.set_cookie("foodID", food)
