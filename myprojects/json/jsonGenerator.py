...
def askValuesAndPairUpWithKeys(keyList, dictList):
    ...
    for item in keyList:
        ...
        while True:
            valueType = input(f">> Is the value of key \"{item}\", single or a list? [0: single, 1: list, 2: JSON fille] : ")
            if not(valueType == "0" or valueType == "1" or  valueType == "2"):
        ...
        if valueType == "0":
            ...
            while True:
                try:
                    importFileName = input(">>>> Please type in the file name(.json is added automatically): ")
                    with open(importFileName + ".json", "r") as jsonFileData:
